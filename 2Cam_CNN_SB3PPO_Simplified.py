...
#         img_size = numLayers * 64 * 64  # 12288
#         image_obs = obs[:, :img_size]  # shape = [1, 12288]

#         # 2) Reshape for CNN
#         image_obs = image_obs.view(-1, numLayers, 64, 64)
...
#         # Known dimensions
#         image_dim = img_dim * img_dim * numLayers  # for 64x64 12,288, for 128x128 196,608
...
#         camview_image = camview_image.view(batch_size, img_dim, img_dim, numLayers)
#         # Convert image to channels-first format (batch_size, 3, img_dim, img_dim)
#         camview_image = camview_image.permute(0, numLayers, 1, 2)
#         # Normalize image
...
#     env=vec_env,
#     verbose=1,
#     tensorboard_log="./runs/ImgPPO_"+envName+"_sb3_simplified"+version+"/",
#     device = device
# )
...
#     log_dir="./runs/ImgPPO_"+envName+"_sb3_simplified"+version+"/tf_logs",
#     log_freq=10000, 
#     verbose=1
# )
...
# model.save("./runs/ImgPPO_"+envName+"_sb3_simplifiedv4/"+envName+"_Img_ppo_reach_simplified"+version)
...
if __name__ == "__main__":
    ...
    model = PPO(
        ...
        env=vec_env,
        verbose=1,
        tensorboard_log=f"./runs/ImgPPO_{envName}_sb3_simplified_{version}/",
        device=device
    )

    # Create the callback
    image_logging_callback = ImageLoggingCallback(
        log_dir=f"./runs/ImgPPO_{envName}_sb3_simplified_{version}/tf_logs",
        log_freq=10000,
        verbose=1
    )
